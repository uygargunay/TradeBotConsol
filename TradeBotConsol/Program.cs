        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    if (key == ConsoleKey.S) // [S] SAFE EXIT
                    {
                    ...
                    if (key == ConsoleKey.P) // [P] PRINT SUMMARY
                    {
                        broker.PrintDailySummary();
                    }

                    if (key == ConsoleKey.K) // [K] EMERGENCY LIQUIDATION
                    {
                        ...
                        broker.SaveState(); // NEW: Save immediately after panic sell
                        Console.WriteLine($"[SYSTEM] Closed {closedTrades.Count} positions. State Saved. Trading Halted.");
                    }
                }

                // --- B. SYSTEM CHECKS ---
                // Triggers the 15-minute progress report even if no prices are moving
                broker.CheckHealth();

                // Checks if it's 3:45 PM ET to wind down for the day
